<div class="">
    <div class="d-flex flex-column align-items-start flex-xxl-row justify-content-between">
        ...
        @if(!empty(getInvoiceNoPrefix()) || !empty(getInvoiceNoSuffix()))
            <div
                    class="d-flex flex-center flex-equal fw-row text-nowrap order-1 order-xxl-2 me-4 align-self-end align-items-center"
                    ...
                    <div class="input-group">
                        @if(!empty(getInvoiceNoPrefix()))
                            <a class="input-group-text bg-secondary border-0 text-decoration-none text-black"
                               data-toggle="tooltip"
                               data-placement="right" title="Invoice No Prefix">
                                {{ getInvoiceNoPrefix() }}
                            </a>
                        @endif
                        ...
                        @if(!empty(getInvoiceNoSuffix()))
                            <a class="input-group-text bg-secondary border-0 text-decoration-none text-black"
                               data-toggle="tooltip"
                               data-placement="right" title="Invoice No Suffix">
                                {{ getInvoiceNoSuffix() }}
                            </a>
                        @endif
                    </div>
    ...
    <div class="mb-0">
        <div class="row gx-10 mb-5">
            <div class="col-lg-3 col-sm-12">
                <div class="mb-5">
                    {{ Form::label('discountType', __('messages.invoice.discount_type').(':'), ['class' => 'form-label mb-3']) }}
                    {{ Form::select('discount_type', $discount_type,0, ['class' =>'form-select io-select2', 'id' => 'discountType','data-control' => "select2"]) }}
                </div>
            </div>
            <div class="col-lg-3 col-sm-12">
                <div class="mb-5">
                    {{ Form::label('discount', __('messages.invoice.discount').(':'), ['class' => 'form-label mb-3']) }}
            ...
            <div class="mb-5 col-lg-6 col-sm-12 recurring-cycle-content">
                ...
                {{ Form::number('recurring_cycle', null, ['class' => 'form-control', 'id' => 'recurringCycle', 'autocomplete' => 'off', 'placeholder'=>'Enter number of Days for Recurring Cycle', 'oninput'=>"validity.valid||(value=value.replace(/[e\+\-]/gi,''))"]) }}
            </div>
        </div>
    </div>
    <div class="mb-0">
                ...
                <tr class="tax-tr">
                    ...
                    <td class="text-end item-total pt-8 text-nowrap">
                        @if(!getSettingValue('currency_after_amount'))<span
                                class="invoice-selected-currency">{{ getCurrencySymbol() }}</span>@endif 0.00
                        @if(getSettingValue('currency_after_amount'))<span
